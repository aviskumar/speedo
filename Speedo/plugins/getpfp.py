...
@speedo.on(Speedo_cmd(pattern="getpic ?(.*)", outgoing=True))
@speedo.on(sudo_cmd(pattern="getpic ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@speedo.on(Speedo_cmd(pattern="poto ?(.*)", outgoing=True))
@speedo.on(sudo_cmd(pattern="poto ?(.*)", allow_sudo=True))
