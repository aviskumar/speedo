...
@speedo.on(Speedo_cmd(pattern=r"getc"))
@speedo.on(sudo_cmd(pattern=r"getc", allow_sudo=True))
...
@speedo.on(Speedo_cmd(pattern=r"geta"))
@speedo.on(sudo_cmd(pattern=r"geta", allow_sudo=True))
async def get_media(event):
    if event.fwd_from:
        return
    dir = "./temp/"
