...
def Speedo_cmd(add_cmd, is_args=False):
    def cmd(func):
        Speedo = bot.tgbot
        if is_args:
...
def is_admin():
    def decorator(func):
        ...
        async def wrapper(event):
            Speedo = bot.tgbot
            perms = await Speedo.get_permissions(event.chat_id, event.sender_id)
...
def is_bot_admin():
    def decorator(func):
        ...
        async def wrapper(event):
            Speedo = bot.tgbot
            boat = await Speedo.get_me()
            perms = await Speedo.get_permissions(event.chat_id, boat)
            if perms.is_admin:
                await func(event)
            else:
